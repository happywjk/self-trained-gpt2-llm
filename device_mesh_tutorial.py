...
def describe_mesh(mesh: "DeviceMesh") -> None:
    ...
    if global_mesh is not None:
        print("global mesh ranks:\n", global_mesh)
    else:
        print("global mesh ranks: (not exposed in this torch version)")
    ...
    if local_mesh is not None:
        print("local mesh ranks:", local_mesh)
    else:
        print("local mesh ranks: (not exposed in this torch version)")
...
def demo_shard(mesh: "DeviceMesh", device_type: str) -> None:
    ...
    if distribute_tensor is None or Shard is None:
        print("sharding demo skipped (torch.distributed._tensor not available)")
        return

    # Simple 1D tensor so the shard split is easy to see.
    x = torch.arange(8, device=device_type)
    x_mesh = distribute_tensor(x, mesh, placements=[Shard(0)])
    # Try to report local rank if API exists; otherwise just print the shard.
...
def project_usage_note(device_type: str, world_size: int) -> None:
    ...
    """
    print("\nProject usage:")
    print(f"- train.py builds a mesh with device_type={device_type}, world_size={world_size}")
    print("- It is printed at startup to confirm creation.")
    print("- You can reuse the same mesh for distributed tensor APIs (e.g., distribute_tensor).")
...
if __name__ == "__main__":
    ...
    if debug_single and local_rank_env != 0:
        print(f"[rank {local_rank_env}] skipping debug path (DEBUG_SINGLE_RANK=1)")
        sys.exit(0)
